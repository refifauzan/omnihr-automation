...
function onEditInstallable(e) {
	try {
		...
		// Skip multi-cell edits (paste operations) to avoid multiple alerts
		const numRows = e.range.getNumRows();
		const numCols = e.range.getNumColumns();
		if (numRows > 1 || numCols > 1) {
			// For multi-cell edits, just log and skip the alert
			// The sync will fix any issues
			Logger.log(
				`Multi-cell edit detected (${numRows}x${numCols}), skipping protection check`,
			);
			return;
		}

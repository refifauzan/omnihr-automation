			...
			for (let day = 1; day <= daysInMonth; day++) {
				...
				if (isWeekend) {
					...
					if (leaveInfo) {
						...
						let capacity = 0;
						// Green = no action needed (0 = fully assigned or on leave). Light red = 1-8 hours lost, actions needed.
						const greenNoAction = '#D4EDDA';
						const lightRedActions = '#FFE6E6';
						...
						if (leaveInfo.is_half_day) {
							...
							bgRow.push(capacity === 0 ? greenNoAction : lightRedActions);
						} else {
							// Full-day leave - no capacity; green (no action needed)
							capacity = 0;
							Logger.log(`  Full-day leave: capacity=0`);
							bgRow.push(greenNoAction);
						}
						...
						// 0 = green (no action needed). 1-8 = light red (actions needed, allocate remaining hours).
						if (capacity === 0) {
							bgRow.push('#D4EDDA'); // Green - fully assigned, no action needed
						} else if (capacity >= 1 && capacity <= 8) {
							bgRow.push('#FFE6E6'); // Light red - actions needed, allocate remaining hours
						} else {
...
/**
 * Setup hourly trigger for capacity view updates
 * Creates a trigger that runs every hour to update all capacity views
 */
function setupHourlyCapacityTrigger() {
	const props = PropertiesService.getScriptProperties();

	// Check if hourly trigger already exists
	const triggers = ScriptApp.getProjectTriggers();
	const hasHourlyTrigger = triggers.some(
		(trigger) =>
			trigger.getHandlerFunction() === 'scheduledHourlyCapacityUpdate',
	);

	if (!hasHourlyTrigger) {
		ScriptApp.newTrigger('scheduledHourlyCapacityUpdate')
			.timeBased()
			.everyHours(1)
			.create();

		props.setProperty('HOURLY_CAPACITY_TRIGGER_ENABLED', 'true');
		Logger.log('Created hourly capacity update trigger');
	} else {
		Logger.log('Hourly capacity trigger already exists');
	}
}

/**
 * Remove hourly trigger for capacity view updates
 */
function removeHourlyCapacityTrigger() {
	const props = PropertiesService.getScriptProperties();
	const triggers = ScriptApp.getProjectTriggers();

	triggers.forEach((trigger) => {
		if (trigger.getHandlerFunction() === 'scheduledHourlyCapacityUpdate') {
			ScriptApp.deleteTrigger(trigger);
			Logger.log('Removed hourly capacity update trigger');
		}
	});
...
/**
 * Scheduled hourly capacity update function
 * Automatically updates all capacity view sheets every hour
 */
function scheduledHourlyCapacityUpdate() {
	const ss = SpreadsheetApp.getActiveSpreadsheet();
	const allSheets = ss.getSheets();
	const capacitySheets = [];

	// Find all capacity view sheets
	for (const sheet of allSheets) {
		const sheetName = sheet.getName();
		if (sheetName.startsWith('CV ')) {
			capacitySheets.push(sheet);
		}
	}

	if (capacitySheets.length === 0) {
		Logger.log('No capacity view sheets found for hourly update');
		return;
	}

	Logger.log(
		`Starting hourly capacity update for ${capacitySheets.length} sheets`,
	);

	let updatedCount = 0;

	for (const capacitySheet of capacitySheets) {
		try {
			const sheetName = capacitySheet.getName();
			const monthYearPart = sheetName.replace('CV ', '');
			const parsed = parseMonthYearFromSheetName(monthYearPart);

			if (!parsed) {
				Logger.log(`Could not parse month/year from sheet: ${sheetName}`);
				continue;
			}

			const { month, year } = parsed;

			// Get project sheets that match this capacity view
			const projectSheetNames = [];
			const monthNames = [
				'January',
				'February',
			...
			for (const sheet of allSheets) {
				...
				if (
					...
					sheet.getLastColumn() >= CONFIG.FIRST_DAY_COL
				) {
					projectSheetNames.push(name);
				}
			}
			...
			if (projectSheetNames.length === 0) {
				Logger.log(`No project sheets found for ${sheetName}`);
				continue;
			}

			// Update the capacity view
			createCapacityViewSheet(month, year, projectSheetNames);
			updatedCount++;
			Logger.log(`Updated capacity view: ${sheetName}`);
		} catch (error) {
			Logger.log(
				`Error updating capacity sheet ${capacitySheet.getName()}: ${error.message}`,
			);
		}
	}

	Logger.log(
		`Hourly capacity update complete: ${updatedCount}/${capacitySheets.length} sheets updated`,
	);
}
